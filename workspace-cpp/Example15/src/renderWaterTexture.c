...
GLUSshaderprogram g_programWaterTexture;

GLint g_projectionMatrixWaterTextureLocation;

GLint g_modelViewMatrixWaterTextureLocation;

GLint g_vertexWaterTextureLocation;

GLint g_texCoordWaterTextureLocation;

GLint g_waterPlaneLengthWaterTextureLocation;

GLint g_passedTimeWaterTextureLocation;

GLint g_waveParametersWaterTextureLocation;

GLint g_waveDirectionsWaterTextureLocation;

//

GLuint g_vaoWaterTexture;

GLuint g_verticesWaterTextureVBO;

GLuint g_texCoordsWaterTextureVBO;

GLuint g_indicesWaterTextureVBO;

GLuint g_numberIndicesWaterTexture;

//

GLuint g_textureWaterTexture;

GLuint g_depthWaterTexture;

//

GLuint g_fboWaterTexture;
...
GLUSuint initWaterTexture(GLUSfloat waterPlaneLength)
{
    ...
    glGenTextures(1, &g_textureWaterTexture);
    glActiveTexture(GL_TEXTURE1);
    glBindTexture(GL_TEXTURE_2D, g_textureWaterTexture);
    ...
    glGenRenderbuffers(1, &g_depthWaterTexture);
    glBindRenderbuffer(GL_RENDERBUFFER, g_depthWaterTexture);
    ...
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, g_textureWaterTexture, 0);

    // ... and the depth buffer,
    glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, g_depthWaterTexture);
...
GLUSvoid terminateWaterTexture(GLUSvoid)
{
    ...
    if (g_textureWaterTexture)
    {
        glDeleteTextures(1, &g_textureWaterTexture);

        g_textureWaterTexture = 0;
    }

    glBindRenderbuffer(GL_RENDERBUFFER, 0);

    if (g_depthWaterTexture)
    {
        glDeleteRenderbuffers(1, &g_depthWaterTexture);

        g_depthWaterTexture = 0;
    }
