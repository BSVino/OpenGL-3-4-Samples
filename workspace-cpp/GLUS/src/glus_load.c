...
GLUSboolean GLUSAPIENTRY glusLoadBinaryFile(const char* filename, GLUSbinaryfile* binaryfile)
{
    FILE* f;
    ...
    binaryfile->binary = (unsigned char*) malloc(binaryfile->length);
    ...
    memset(binaryfile->binary, 0, binaryfile->length);

    rewind(f);

    fread(binaryfile->binary, 1, binaryfile->length, f);
